 ...
 * operator_id: 經營者id type String or Number(字串或數字)
 ...
 * @example {
 *   operator_id: ype Number or String(字串或數字) 經營者id
 ...
 * @example {
 *   operatorId: type String or Number(字串或數字) 經營者id
 ...
 * @param { type Object(物件) } data
 * @example {
 *    month: type String or Number(字串或數字) 月份
 *    day: type String or Number(字串或數字) 日期
 *    sendSmsActive: type Boolean(布林值 ex: 0 或 1) 是否禁止發送簡訊對象
 * }
 */
export const SendSmsMemberListsApi = (data) => {
    return axios.put(`${apiConfig}/userMember/smsSendLists`, data);
};
/**
 * 模糊比對取得管理裡者列表
 * @param { type Object(物件) } data 搜尋資料
 * @example {
 *   custom_id: type String(字串) 客製化編號
 *   account: type String(字串) 帳號
 *   name: type String(字串) 名稱
 *   nickname: type String(字串) 暱稱
 * }
 */
 export const MemberRemoteListsApi = (data) => {
    return axios.put(`${apiConfig}/userMember/remoteLists`, data);
};
