        ...
        <div v-loading="loading">
            <el-table :data="datas"
                ...
                <el-table-column prop="status"
                                 ...
                                 min-width="200">
                    <template v-slot="scope">
                        <div class="bg-indigo-50 p-2 rounded-lg inline-block">
...
/**
 * AssistantUserListApi = 取得助理列表
 * AssistantUserListByIdApi = 取得指定id助理列表
 * UpdateAssistantApi = 更新助理
 */
import { AssistantUserListApi, AssistantUserListByIdApi, UpdateAssistantApi } from "../../../api/assistantApi";
// 導入搜尋條件判斷式
...
export default {
    ...
    methods: {
        ...
        resetData() {
            ...
            if (self.$route.params.id !== undefined) {
                self.getListsById({
                    ...
                    sort: ["assistant_id|DESC"]
                });
            } else {
                self.getLists({ sort: ["assistant_id|DESC"] });
            }
        },
        /**
        ...
        filterSearch() {
            let self = this;
            checkHaveSelectData([["assistant_id|DESC"], self.startTime, self.endTime, self.searchAccount, self.searchStatus], ["sort", "startDateTime", "endDateTime", "account", "status"], self.filterData);
            // 搜尋完後將分頁改回第一頁
            self.currentPage = 1;
            // 搜尋完後將分頁改回第一頁
         ...
         * @example { user_id: users表id assistant_id: user_assistants表id, operator_id: 隸屬經營者id, status: 狀態值 }
        ...
        async onActive(data, index) {
            ...
            try {
                ...
                if (value) {
                    try {
                        await UpdateAssistantApi(sendData);
                        self.$message({
                            type: "success",
                            message: "更新成功"
                        });
        ...
        async getLists(sendData, pageSize = false) {
            ...
            self.loading = true;
            try {
                let { data } = await AssistantUserListApi(sendData);
        ...
        async getListsById(sendData, pageSize = false) {
            ...
            self.loading = true;
            try {
                let { data } = await AssistantUserListByIdApi(sendData);
                self.datas = data.data.data;
                self.currentPage = data.data.current_page;
                self.paginationData.total = data.data.total;
                // 當pageSize 為 true 時 將預設一頁呈現幾筆值 改為傳入參數值
