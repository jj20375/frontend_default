<template>
    <div>
        ...
        <button class="blue-btn float-right"
        ...
        <el-table :data="datas"
            ...
            <el-table-column cell-class-name="whitespace-nowrap"
                             label="附加功能">
                <template v-slot="scope">
                    <div v-if="haveOptions[scope.row.key]">
                        <label class="ml-2" :for="'option'+scope.row.key+key" v-for="(option, key) in haveOptions[scope.row.key]" :key="key">
                            <input :id="'option'+scope.row.key+key" type="checkbox" :value="option" v-model="checkOptions[scope.row.key]" />
                            {{option.name}}
                        </label>
                    </div>
                </template>
...
/**
 * UpdateUserPermissionApi = 更新角色群組權限
 * UserTreePermissionApi = 樹狀權限資料
 * UserPermissionListsApi = 權限列表資料
 * UserPermissionCRUDApi = 權限CRUD資料
 */
import { UserTreePermissionApi, UserPermissionListsApi, UserPermissionCRUDApi, UpdateUserPermissionApi } from "../../../api/permissionUserApi";
...
export default {
    ...
    methods: {
        ...
        async getPermissionTreeLists() {
            ...
            try {
                let { data } = await UserTreePermissionApi(groupCode);
        ...
        async getPermissionLists() {
            ...
            let groupCode = self.$route.params.groupCode;
            try {
                let { data } = await UserPermissionListsApi(groupCode);
        ...
        async getPermissionCRUDLists() {
            ...
            let groupCode = self.$route.params.groupCode;
            try {
                let { data } = await UserPermissionCRUDApi(groupCode, groupId);
        ...
        async onSubmit() {
            ...
            result.forEach((item, index) => {
                Object.keys(self.checkOptions).map((objKey) => {
                    if (item.key === objKey) {
                        result[index].options = self.checkOptions[objKey].length <= 0 ? null : self.checkOptions[objKey];
                    }
                });
                // 比對 新增功能 權限key
            ...
            try {
                await UpdateUserPermissionApi(sendData);
                self.$message({
                    type: "success",
                    message: "更新成功"
                });
        ...
        backClick() {
            let self = this;
            switch (true) {
                case self.$route.params.groupCode === "OPERSTOR":
                    self.$router.push({ name: "accountControl_operator" });
                    break;
                case self.$route.params.groupCode === "DESIGNER":
                    self.$router.push({ name: "accountControl_designer" });
                    break;
                case self.$route.params.groupCode === "ASSISTANT":
                    self.$router.push({ name: "accountControl_assistant" });
                    break;
                case self.$route.params.groupCode === "OPERATOR" && self.$route.params.isSub === 1:
                    self.$router.push({ name: "accountControl_sub" });
                    break;
                default:
                    self.$router.push({ name: "accountControl_operator" });
                    break;
