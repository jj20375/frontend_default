<template>
    <div>
        <div>
            <ul class="inline-block mb-2 bg-indigo-50 rounded-lg py-2">
                <li class="pl-2 pr-2 inline-block cursor-pointer"
                    @click="breadcrumbClick(item, index)"
                    v-for="(item,index) in parent"
                    :key="index"
                    :class="[(index + 1) === parent.length && 'text-blue-700']">
                    {{item.name}}
                    <span v-if="index +1 !== parent.length"><i class="fas fa-chevron-right"></i></span>
                </li>
            </ul>
        </div>
        <div class="float-left">
            <button class="blue-btn-outline mr-2"
                    v-if="parent.length !==0"
                    @click="backDepth()">返回上層
            </button>
            <button class="green-btn mr-2"
        ...
        <div class="float-right">
            <span class="bg-blue-900 p-2 text-white rounded-lg">目前層級：{{ depth }}</span>
            <span class="bg-red-700 p-2 text-white rounded-lg">目前用戶: {{ operator.name }}</span>
        </div>
        <div class="clear-both"></div>
        <el-table :data="datas"
                  header-cell-class-name="text-left align-middle theadBg text-white"
                  class="w-full tableBg"
            ...
            <el-table-column prop="name"
                ...
                <template v-slot="scope">
                    <div v-loading="loading">
                        <div class="mb-2">
                            <h5 class=" mb-0 text-blue-600 border-b border-blue-600 font-bold cursor-pointer"
                                :disabled="loading"
                                @click="changeAgent(scope.row)">
                                {{ scope.row.account }}&nbsp;<i class="fas fa-external-link-alt"></i>
                            </h5>
                        </div>
                        <h5 class="m-0 text-red-500 font-normal"
            ...
            <el-table-column label="時間"
                             min-width="200">
                <template v-slot="scope">
                    <div class="text-gray-600">創建時間:<span class="ml-2">{{ scope.row.created_at | formatDateTime }}</span></div>
                    <div class="text-indigo-600 font-medium">最後登入:<span class="ml-2">{{ scope.row.user.login_time | formatUnixDate }}</span></div>
                </template>
            </el-table-column>
            <el-table-column label="登入IP"
                             min-width="200">
                <template v-slot="scope">
                    <div class="text-gray-600">創建IP:<span class="ml-2">{{ scope.row.user.create_ip }}</span></div>
                    <div class="text-red-800 font-medium">最後登入IP:<span class="ml-2">{{ scope.row.user.last_ip }}</span></div>
                </template>
            </el-table-column>
            <el-table-column prop="status"
                             ...
                             min-width="200">
                <template v-slot="scope">
                    <div class="p-2 bg-indigo-50 inline-block rounded-lg">
            ...
            <el-table-column label="設置"
                             min-width="150">
                <template v-slot="scope">
                    <button class="indigo-btn-outline-sm ml-1 mt-2"
                            v-if="checkShow('accountControl_operator_permissionSet')"
                            @click="$router.push({name: 'accountControl_operator_permissionSet', params:{id: scope.row.user.user_id, groupCode: scope.row.user.group.group_code}})">權限</button>
                    <button class="indigo-btn-outline-sm ml-1 mt-2"
                            v-if="checkShow('accountControl_sub')"
                            @click="$router.push({name: 'accountControl_sub', params:{type: 'OPERATOR', userId: scope.row.user.user_id, operatorId: scope.row.operator_id}})">子帳號</button>
                    <button class="indigo-btn-outline-sm ml-1 mt-2"
                            v-if="checkShow('accountControl_assistant')"
                            @click="$router.push({name: 'accountControl_assistant', params:{id: scope.row.operator_id}})">助理</button>
                    <button class="indigo-btn-outline-sm ml-1 mt-2"
                            v-if="checkShow('accountControl_designer')"
                            @click="$router.push({name: 'accountControl_designer', params:{id: scope.row.operator_id}})">服務提供者</button>
                    <button class="indigo-btn-outline-sm ml-1 mt-2"
                            v-if="checkShow('accountControl_member')"
                            @click="$router.push({name: 'accountControl_member', params:{operatorId: scope.row.operator_id, operatorAccount: scope.row.account}})">會員</button>
                    <button class="indigo-btn-outline-sm ml-1 mt-2"
                            v-if="checkShow('webControl_storeService')"
                            @click="$router.push({name: 'webControl_storeService', params:{operatorId: scope.row.operator_id}})">技術</button>
                    <button class="indigo-btn-outline-sm ml-1 mt-2"
                            v-if="checkShow('commodity_provider_list')"
                            @click="$router.push({name: 'commodity_provider_list', params:{operatorId: scope.row.operator_id}})">廠商</button>
                    <button class="indigo-btn-outline-sm ml-1 mt-2"
                            v-if="checkShow('commodity_product_list')"
                            @click="$router.push({name: 'commodity_product_list', params:{operatorId: scope.row.operator_id}})">產品</button>
                    <button class="indigo-btn-outline-sm ml-1 mt-2"
                            v-if="checkShow('orderControl_purchase_order_list')"
                            @click="$router.push({name: 'orderControl_purchase_order_list', params:{operatorId: scope.row.operator_id}})">進貨訂單</button>
                    <button class="indigo-btn-outline-sm ml-1 mt-2"
                            v-if="checkShow('commodity_list')"
                            @click="$router.push({name: 'commodity_list', params:{operatorId: scope.row.operator_id}})">商品</button>
                    <button class="indigo-btn-outline-sm ml-1 mt-2"
                            v-if="checkShow('orderControl_operator_order_list')"
                            @click="$router.push({name: 'orderControl_operator_order_list', params:{operatorId: scope.row.operator_id }})">商家訂單</button>
                    <button class="indigo-btn-outline-sm ml-1 mt-2"
                            v-if="checkShow('otherControl_salary')"
                            @click="$router.push({name: 'otherControl_salary', params:{operatorId: scope.row.operator_id }})">薪水</button>
                </template>
            </el-table-column>
...
/**
 * OperatorUserListApi = 取得經營者層級列表 api
 * UpdateOperatorStatusApi = 更新經營者狀態值 api
 */
...
export default {
    ...
    methods: {
        ...
        /**
         * 將目前經營者層級一層一層加進陣列中 length 數字 等於 目前總共層數
         * @param { type Object(物件) } data 傳入上層經營者資料
         */
        addParent(data) {
            let self = this;
        ...
        backDepth() {
            ...
            // 返回上層時 將上層經營者陣列值 移除最後一位
            self.parent = _.dropRight(self.parent, 1);
            // 如果上層經營者剩下最上層時，清空上層經營者陣列
        ...
        /**
         * 取得經營者層級列表
         * @param { type Number or String(數字或字串) } operatorId 需要取得的經營者id operator_id
         * @param { type Object(物件) } operatorDatga 上層經營者的資料
         */
        ...
        /**
         * 更新 當前經營者資料
         * @param { type String or Number(數字或字串) } currentOperatorId 新增經營者時要用的id
