            ...
            <el-table :data="datas"
                ...
                <el-table-column label="會員"
                    ...
                    <template v-slot="scope">
                        <div>
                            ...
                            <h5 class="mb-0 text-blue-700">客戶編號:<span class="ml-2">{{ scope.row.user_member_info.custom_id }}</span></h5>
                            <h5 class="mb-0 text-red-700 font-medium">名稱:<span class="ml-2">{{ scope.row.name }}</span></h5>
                            <h5 class="mb-0 text-indigo-600 font-medium">暱稱:<span class="ml-2">{{ scope.row.nickname }}</span></h5>
                            <div class="font-medium hot-color">生日:<span class="ml-2">{{ scope.row.birthday | formatDate }}</span></div>
                            <h5 class="font-medium green-color mb-0">點數:<span class="ml-2">{{ scope.row.user_member_info === null ? 0 : scope.row.user_member_info.point | formatCurrency }}</span></h5>
                        </div>
                    </template>
                </el-table-column>
                <el-table-column min-width="150"
                    ...
                    <template v-slot="scope">
                        ...
                        <div class="font-medium text-blue-500">名稱:<span class="ml-2">{{ scope.row.user_operator.name }}</span></div>
                    </template>
                </el-table-column>
                <el-table-column label="時間/登入ip"
                    ...
                    <template v-slot="scope">
                        ...
                        <div class="text-indigo-600 border-b border-gray-400">最後登入:<div class="ml-2">{{ scope.row.user.login_time | formatUnixDate }}</div>
                        </div>
...
// 導入模糊搜尋會員輸入匡
import RemoteMemberInput from "../../components/RemoteMemberInput.vue";
export default {
    ...
    methods: {
        ...
        filterSearch() {
            let self = this;
            checkHaveSelectData([["member_id|DESC"], self.startTime, self.endTime, self.searchAccount, self.searchStatus, self.searchName, self.searchNickName, self.searchCustomId], ["sort", "startDateTime", "endDateTime", "account", "status", "name", "nickname", "whereHasLike|userMemberInfo|custom_id"], self.filterData);
            // 搜尋完後將分頁改回第一頁
            self.currentPage = 1;
            // 搜尋完後將分頁改回第一頁
