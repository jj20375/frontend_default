<template>
    <el-dialog :title="`${submitText}會員`"
        ...
        <el-form :label-position="'right'"
            ...
            <el-form-item label="手機2">
                <el-input v-model="form.phone2"
                          placeholder="請輸入手機2號碼"></el-input>
            </el-form-item>
            <el-form-item label="是否發送簡訊">
                <el-switch style="display: block;"
                           v-model="form.sendSmsActive"
                           active-color="#2dce89"
 ...
 */
import { AddMemberApi, UpdateMemberApi, MemberDataApi } from "../../../api/memberApi";
export default {
    props: {
        // 外部組件 傳值開啟彈窗
        showDialog: {
    ...
    methods: {
        ...
        async add(form) {
            ...
            if (self.user.group.group_code === "SYSTEM") {
                sendData.account = self.$route.params.operatorAccount + "_" + sendData.custom_id;
                sendData.password = self.$route.params.operatorAccount + "&" + sendData.custom_id;
            } else {
                sendData.account = self.user.userable.user_operator.account + "_" + sendData.custom_id;
                sendData.password = self.user.userable.user_operator.account + "&" + sendData.custom_id;
            }
            try {
                await AddMemberApi(sendData);
                self.$message({
                    type: "success",
                    message: "新增成功"
                });
        ...
        async getData(account) {
            ...
            try {
                ...
                self.form = {
                    ...
                    sendSmsActive: data.data.sendSmsActive,
