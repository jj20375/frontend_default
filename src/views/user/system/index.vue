<template>
    <div>
        ...
        <div v-loading="loading">
            <el-table :data="datas"
                      header-cell-class-name="text-left theadBg align-middle text-white"
                      class="tableBg"
                ...
                <el-table-column label="名稱"
                    ...
                    <template v-slot="scope">
                        <div>
                            <h5 class="mb-0 text-blue-600">帳號:<span class="ml-2">{{ scope.row.account }}</span></h5>
                            <h5 class="mb-0 text-blue-800 font-medium">名稱:<span class="ml-2">{{ scope.row.name }}</span></h5>
                        </div>
                    </template>
                </el-table-column>
                <el-table-column label="群組"
                    ...
                    <template v-slot="scope">
                        <div>
                            <h5 class="mb-0 text-red-600">類別:<span class="ml-2">{{ scope.row.user.group.group_code }}</span></h5>
                            <h5 class="mb-0 text-gray-400 font-medium">名稱:<span class="ml-2">{{ scope.row.user.group.group_name }}</span></h5>
                        </div>
                    </template>
                </el-table-column>
                <el-table-column label="時間"
                                 min-width="200">
                    <template v-slot="scope">
                        <div class="text-gray-600">創建時間:<div class="ml-2">{{ scope.row.created_at | formatDateTime }}</div>
                        </div>
                        <div class="text-indigo-600 font-medium">最後登入:<div class="ml-2">{{ scope.row.user.login_time | formatUnixDate }}</div>
                        </div>
                    </template>
                </el-table-column>
...
export default {
    ...
    methods: {
        ...
        filterSearch() {
            let self = this;
            checkHaveSelectData([["system_id|DESC"], self.startTime, self.endTime, self.searchAccount, self.searchStatus], ["sort", "startDateTime", "endDateTime", "account", "status"], self.filterData);
            // 搜尋完後將分頁改回第一頁
            self.currentPage = 1;
            // 搜尋完後將分頁改回第一頁
