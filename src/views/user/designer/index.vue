<template>
    <div>
        <button class="blue-btn"
                v-if="$route.meta.params.Create === 1 && (user.group.group_code !== 'SYSTEM' || $route.params.id !== undefined)"
                @click="clickDialog()">
            ...
            <el-table :data="datas"
                ...
                <el-table-column label="名稱"
                    ...
                    <template v-slot="scope">
                        <div>
                            <h5 class="mb-0 text-blue-700">
                                帳號:<span class="ml-2">{{ scope.row.account }}</span>
                            </h5>
                            <h5 class="mb-0 text-gray-600 font-medium">
                                名稱:<span class="ml-2">{{ scope.row.name }}</span>
                            </h5>
                            <h5 class="mb-0 text-indigo-600 font-medium">
                                暱稱:<span class="ml-2">{{ scope.row.nickname }}</span>
                            </h5>
                            <h5 class="mb-0 text-red-700 font-medium"
                                v-if="user.group.group_code === 'SYSTEM'">
                                經營者帳號:<span class="ml-2">{{ scope.row.user_operator.account }}</span>
                            </h5>
                        </div>
                    </template>
                </el-table-column>
                <el-table-column label="群組"
                    ...
                    <template v-slot="scope">
                        <div>
                            <h5 class="mb-0 text-blue-700">
                                類別:<span class="ml-2">{{ scope.row.user.group.group_code }}</span>
                            </h5>
                            <h5 class="mb-0 text-gray-600 font-medium">
                                名稱:<span class="ml-2">{{ scope.row.user.group.group_name }}</span>
                            </h5>
                        </div>
                    </template>
                </el-table-column>
                <el-table-column label="時間/登入ip"
                                 min-width="200">
                    <template v-slot="scope">
                        <div class="text-gray-600 border-b border-gray-400">
                            創建時間:
                            <div class="ml-2">
                                {{ scope.row.created_at | formatDateTime }}
                            </div>
                        </div>
                        <div class="text-indigo-600 font-medium border-b border-gray-400">
                            最後登入:
                            <div class="ml-2">
                                {{ scope.row.user.login_time | formatUnixDate }}
                            </div>
                        </div>
                        <div class="text-gray-600 border-b border-gray-400">
                            創建IP:<span class="ml-2">{{ scope.row.user.create_ip }}</span>
                        </div>
                        <div class="text-red-800 font-medium border-b border-gray-400">
                            最後登入IP:<span class="ml-2">{{ scope.row.user.last_ip }}</span>
                        </div>
                    </template>
                </el-table-column>
                ...
                <el-table-column label="設置"
                    ...
                    <template v-slot="scope">
                        <button class="indigo-btn-outline-sm ml-1 mt-2"
                                v-if="checkShow('accountControl_designer_panel')"
                                @click="goToPanel(scope.row)">控制台</button>
                        <button class="indigo-btn-outline-sm ml-1 mt-2"
...
/**
 * DesignerUserListApi = 取得服務提供者列表
 * DesignerUserListByIdApi = 取得指定id服務提供者列表
 * UpdateDesignerApi = 更新服務提供者
 */
import { DesignerUserListApi, DesignerUserListByIdApi, UpdateDesignerApi } from "../../../api/designerApi";
// 導入搜尋條件判斷式
...
export default {
    ...
    methods: {
        ...mapMutations("designerStore", ["setDesignerName"]),
        // 自定義索引
        indexMethod(val) {
            let self = this;
        ...
        /**
         * 跳轉服務提供者平台
         * @param { type Object(物件) } data 選中行資料
         */
        goToPanel(data) {
            let self = this;
            self.$router.push({
                ...
                params: {
                    ...
                    operatorId: data.user_operator.operator_id,
                    designerId: data.designer_id
                }
            });
            self.setDesignerName(data.name);
        },
        /**
         * 檢查是否要顯示功能按鈕(暫時沒用到)
        ...
        resetData() {
            ...
            if (self.$route.params.id !== undefined) {
                self.getListsById({
                    operatorId: self.$route.params.id,
                    sort: ["designer_id|DESC"]
                });
            } else {
                self.getLists({ sort: ["designer_id|DESC"] });
            }
        },
        /**
        ...
        paginationAjax(val, pageSize) {
            ...
            if (self.$route.params.id !== undefined) {
                sendData.operatorId = self.$route.params.id;
        ...
        filterSearch() {
            let self = this;
            checkHaveSelectData([["designer_id|DESC"], self.startTime, self.endTime, self.searchAccount, self.searchStatus], ["sort", "startDateTime", "endDateTime", "account", "status"], self.filterData);
            // 搜尋完後將分頁改回第一頁
            self.currentPage = 1;
            // 搜尋完後將分頁改回第一頁
            self.filterData.page = 1;
            if (self.$route.params.userId !== undefined) {
                self.filterData.operatorId = self.$route.params.id;
        ...
        onEdit(data) {
            ...
            // 經營者id
            self.operatorId = data.user_operator.operator_id;
        },
        // 關閉註冊彈窗
        closeDialog() {
            let self = this;
         ...
         * @example { user_id: users表id designer_id: user_designers表id, operator_id: 隸屬經營者id, status: 狀態值 }
        ...
        async onActive(data, index) {
            ...
            try {
                ...
                if (value) {
                    try {
                        await UpdateDesignerApi(sendData);
                        self.$message({
                            type: "success",
                            message: "更新成功"
                        });
        ...
        async getLists(sendData, pageSize = false) {
            ...
            self.loading = true;
            try {
                let { data } = await DesignerUserListApi(sendData);
        ...
        async getListsById(sendData, pageSize = false) {
            ...
            self.loading = true;
            try {
                let { data } = await DesignerUserListByIdApi(sendData);
                self.datas = data.data.data;
                self.currentPage = data.data.current_page;
                self.paginationData.total = data.data.total;
                // 當pageSize 為 true 時 將預設一頁呈現幾筆值 改為傳入參數值
