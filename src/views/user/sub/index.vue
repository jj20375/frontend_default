            ...
            <el-table :data="datas"
                ...
                <el-table-column label="名稱"
                    ...
                    <template v-slot="scope">
                        <div>
                            <h5 class="mb-0 text-blue-700">帳號:<span class="ml-2">{{ scope.row.account }}</span></h5>
                            <h5 class="mb-0 text-gray-600 font-medium">名稱:<span class="ml-2">{{ scope.row.name }}</span></h5>
                            <h5 class="mb-0 text-red-700 font-medium">隸屬帳號:<span class="ml-2">{{ scope.row.subable.account }}</span></h5>
                            <h5 class="mb-0 text-indigo-600 font-medium">隸屬身份:<span class="ml-2">{{ scope.row.user.group.group_code }}</span></h5>
                        </div>
                    </template>
                </el-table-column>
                <el-table-column label="群組"
                    ...
                    <template v-slot="scope">
                        <div>
                            <h5 class="mb-0 text-blue-700">類別:<span class="ml-2">{{ scope.row.user.group.group_code }}</span></h5>
                            <h5 class="mb-0 text-gray-600 font-medium">名稱:<span class="ml-2">{{ scope.row.user.group.group_name }}</span></h5>
                        </div>
                    </template>
                </el-table-column>
                <el-table-column label="時間/登入ip"
                                 min-width="200">
                    <template v-slot="scope">
                        <div class="text-gray-600 border-b border-gray-400">創建時間:<div class="ml-2">{{ scope.row.created_at | formatDateTime }}</div>
                        </div>
                        <div class="text-indigo-600 font-medium border-b border-gray-400">最後登入:<div class="ml-2">{{ scope.row.user.login_time | formatUnixDate }}</div>
                        </div>
                        <div class="text-gray-600 border-b border-gray-400">創建IP:<span class="ml-2">{{ scope.row.user.create_ip }}</span></div>
                        <div class="text-red-800 font-medium border-b border-gray-400">最後登入IP:<span class="ml-2">{{ scope.row.user.last_ip }}</span></div>
                    </template>
                </el-table-column>
                ...
                <el-table-column label="設置"
                    ...
                    <template v-slot="scope">
                        <button class="indigo-btn-outline-sm ml-1 mt-2"
                                ...
                                @click="$router.push({name: 'accountControl_operator_permissionSet', params:{id: scope.row.user.user_id, groupCode: scope.row.user.group.group_code, isSub: 1}})">權限</button>
                    </template>
                </el-table-column>
...
// 導入日期套件需要用到的方法
import DatePickerMixin from "../../../service/DatePickerService";
// 導入分頁組件
import MyPagination from "../../components/MyPagination.vue";
/**
 * SubUserListApi = 取的子帳號列表 api
 * SubUserListByIdApi = 取的指定id子帳號列表 api
 * UpdateSubApi = 更新子帳號 api
 */
import { SubUserListApi, SubUserListByIdApi, UpdateSubApi } from "../../../api/subApi";
// 導入搜尋條件判斷式
    ...
    methods: {
        ...
        resetData() {
            ...
            if (self.$route.params.userId !== undefined) {
                self.getListsById({ userId: self.$route.params.userId, sort: ["sub_id|DESC"] });
            } else {
                self.getLists({ sort: ["sub_id|DESC"] });
            }
        },
        /**
        ...
        paginationAjax(val, pageSize) {
            ...
            if (self.$route.params.userId !== undefined) {
                sendData.userId = self.$route.params.userId;
        ...
        filterSearch() {
            let self = this;
            checkHaveSelectData([["sub_id|DESC"], self.startTime, self.endTime, self.searchAccount, self.searchStatus], ["sort", "startDateTime", "endDateTime", "account", "status"], self.filterData);
            // 搜尋完後將分頁改回第一頁
            self.currentPage = 1;
            // 搜尋完後將分頁改回第一頁
        ...
        clickDialog() {
            ...
            // 判斷網址上是否有 type 參數 如果沒有則抓取 登入者身份的群組代碼
            self.groupCode = self.$route.params.type === undefined ? self.user.group.group_code : self.$route.params.type;
        },
        /**
         * 打開更新彈窗 並傳入選中的資料
        ...
        onEdit(data) {
            ...
            // 判斷點擊的子帳號身份是為經營者的子帳號 還是系統的子帳號
            if (data.user.group.group_code === "OPERATOR") {
                // 經營者身份的子帳號 回傳經營者id
                self.operatorId = data.subable.operator_id;
            } else {
                // 系統身份的子帳號 回傳 0
                self.operatorId = 0;
            }
            // 抓取點擊的子帳號群組代碼
            self.groupCode = data.user.group.group_code;
        },
        // 關閉註冊彈窗
        closeDialog() {
            let self = this;
         ...
         * @example { user_id: users表id sub_id: user_subs表id, status: 狀態值}
        ...
        async getLists(sendData, pageSize = false) {
            ...
            self.loading = true;
            try {
                let { data } = await SubUserListApi(sendData);
        ...
        async getListsById(sendData, pageSize = false) {
            ...
            self.loading = true;
            try {
                let { data } = await SubUserListByIdApi(sendData);
                self.datas = data.data.data;
                self.currentPage = data.data.current_page;
                self.paginationData.total = data.data.total;
                // 當pageSize 為 true 時 將預設一頁呈現幾筆值 改為傳入參數值
