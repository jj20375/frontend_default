        ...
        <el-table :data="datas"
            ...
            <el-table-column min-width="200"
                ...
                <template v-slot="scope">
                    <div class="font-medium text-blue-700">名稱:<span class="ml-2">{{ scope.row.name }}</span></div>
                    <div v-if="user.group.group_code === 'SYSTEM'"
                         class="font-medium text-gray-600">
                        隸屬經營者:<span class="ml-2">{{ scope.row.operator.account }}({{ scope.row.operator.name }})</span>
                    </div>
                </template>
            </el-table-column>
            <el-table-column min-width="150"
                             label="分類">
                <template v-slot="scope">
                    <div class="font-medium text-blue-700">名稱:<span class="ml-2">{{ scope.row.store_service.category.name }}</span></div>
                    <div class="font-medium text-red-700">代碼:<span class="ml-2">{{ scope.row.store_service.category.key }}</span></div>
                </template>
            </el-table-column>
            <el-table-column min-width="150"
                             label="技術創建者">
                <template v-slot="scope">
                    <div class="font-medium text-red-700">帳號:<span class="ml-2">{{ scope.row.serviceable.name }}</span></div>
                    <div class="font-medium text-blue-700">名稱:<span class="ml-2">{{ scope.row.serviceable.account }}</span></div>
                </template>
            </el-table-column>
            <el-table-column min-width="100"
                             label="價位">
                <template v-slot="scope">
                    <div class="font-medium text-red-700">{{ scope.row.store_service.store_service_info !== null ? scope.row.store_service.store_service_info.price : '尚未設定' | formatCurrency }}</div>
                </template>
            </el-table-column>
