        ...
        <el-table :data="datas"
            ...
            <el-table-column min-width="150"
                ...
                <template v-slot="scope">
                    <div class="font-medium text-blue-700">{{ scope.row.order_number }}</div>
                </template>
            </el-table-column>
            <el-table-column min-width="150"
                             label="點數紀錄">
                <template v-slot="scope">
                    <div class="font-medium text-gray-600">更新前:<span class="ml-2">{{ scope.row.before_point | formatCurrency }}</span></div>
                    <div class="font-medium text-blue-700">新增:<span class="ml-2">{{ scope.row.point | formatCurrency }}</span></div>
                    <div class="font-medium text-red-700">更新後:<span class="ml-2">{{ scope.row.after_point | formatCurrency }}</span></div>
                </template>
            </el-table-column>
            <el-table-column min-width="150"
                             label="會員">
                <template v-slot="scope">
                    <div class="font-medium text-blue-700">名稱:<span class="ml-2">{{ scope.row.user_member.name }}</span></div>
                    <div class="font-medium text-red-700">帳號:<span class="ml-2">{{ scope.row.user_member.account }}</span></div>
                </template>
            </el-table-column>
            <el-table-column min-width="150"
                             label="操作人員">
                <template v-slot="scope">
                    <div class="font-medium text-blue-700">名稱:<span class="ml-2">{{ scope.row.user.userable.name }}</span></div>
                    <div class="font-medium text-red-700">帳號:<span class="ml-2">{{ scope.row.user.userable.account }}</span></div>
                </template>
            </el-table-column>
            ...
            <el-table-column min-width="150"
                             label="創建與更新時間">
                <template v-slot="scope">
                    <div class="font-medium text-gray-600">創建時間:<span class="ml-2">{{ scope.row.created_at | formatDateTime }}</span></div>
                    <div class="font-medium text-indigo-600">更新時間:<span class="ml-2">{{ scope.row.updated_at | formatDateTime }}</span></div>
                </template>
            </el-table-column>
        </el-table>
...
/**
 * PointOrderListApi = 取得點數訂單列表api
 * PointOrderListByIdApi = 取得指定id點數訂單列表api
 */
import { PointOrderListApi, PointOrderListByIdApi } from "../../../api/pointOrderApi";
...
export default {
    ...
    methods: {
        ...
        resetData() {
            ...
            if (self.$route.params.operatorId !== undefined) {
                self.getListsById({
                    ...
                    sort: ["point_order_id|DESC"]
                });
            } else {
                self.getLists({ sort: ["point_order_id|DESC"] });
            }
        },
        // 搜尋條件
        filterSearch() {
            let self = this;
            // 判斷是否有值 有值就帶入搜尋條件
            checkHaveSelectData(["point_order_id|DESC", self.searchOrder, self.searchAccount, self.startTime, self.endTime], ["sort", "order_number", "whereHas|userMember|account", "startDateTime", "endDateTime"], self.filterData);
            // 搜尋完後將分頁改回第一頁
            self.currentPage = 1;
            // 搜尋完後將分頁改回第一頁
        ...
        async getLists(sendData, pageSize = false) {
            ...
            self.loading = true;
            try {
                let { data } = await PointOrderListApi(sendData);
        ...
        async getListsById(sendData, pageSize = false) {
            ...
            self.loading = true;
            try {
                let { data } = await PointOrderListByIdApi(sendData);
                self.datas = data.data.data;
                self.currentPage = data.data.current_page;
                self.paginationData.total = data.data.total;
                // 當pageSize 為 true 時 將預設一頁呈現幾筆值 改為傳入參數值
