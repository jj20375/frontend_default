<template>
    <div>
        ...
        <div class="grid grid-cols-12 gap-4">
            ...
            <div class="md:col-span-3 col-span-12">
                <label for="">標籤代碼</label>
                <el-input placeholder="請輸入標籤代碼"
                          v-model="searchKey"
                          clearable
                          prefix-icon="el-icon-search">
                </el-input>
        ...
        <el-table :data="datas"
            ...
            <el-table-column min-width="150"
                             label="標籤創建者">
                <template v-slot="scope">
                    <div class="font-medium text-red-700">帳號:<span class="ml-2">{{ scope.row.createuser_able.account }}</span></div>
                    <div class="font-medium text-blue-700">名稱:<span class="ml-2">{{ scope.row.createuser_able.name }}</span></div>
                </template>
            </el-table-column>
                ...
                <template v-slot="scope">
                    <div class="p-2 bg-indigo-50 rounded-lg inline-block">
                        <el-switch style="display: block;"
                                   ...
                                   :disabled="$route.meta.params.Update == 0 || (user.group.group_code !== 'SYSTEM' && user.userable.user_operator.account !== scope.row.createuser_able.account)"
                                   active-color="#2dce89"
            ...
            <el-table-column label="操作"
                ...
                <template v-slot="scope">
                    <button type="button"
                            ...
                            v-if="$route.meta.params.Update === 1 && (user.group.group_code === 'SYSTEM' || user.userable.user_operator.account === scope.row.createuser_able.account)"
...
/**
 * TagListApi = 取得標籤列表api
 * UpdateTagApi = 更新標籤 api
 */
import { TagListApi, UpdateTagApi } from "../../api/tagApi";
...
export default {
    ...
    methods: {
         ...
         * @example { tag_id: categories表id, active: 啟用狀態 }
        ...
        filterSearch() {
            let self = this;
            // 判斷是否有值 有值就帶入搜尋條件
            checkHaveSelectData(["tag_id|DESC", self.searchName, self.searchKey, self.startTime, self.endTime], ["sort", "name", "key", "startDate", "endDate"], self.filterData);
            // 搜尋完後將分頁改回第一頁
            self.currentPage = 1;
            // 搜尋完後將分頁改回第一頁
        ...
        resetData() {
            ...
            self.getLists({ sort: ["tag_id|DESC"] });
        },
        // 點擊新增時看起的談窗
        clickDialog() {
            let self = this;
        ...
        async getLists(sendData, pageSize = false) {
            ...
            self.loading = true;
            try {
                let { data } = await TagListApi(sendData);
